...
if (form_isset("submitted_all")) {
    mysql_query("UPDATE $tab[mail] SET del='yes' WHERE inbox='transfers' AND dest='$id' AND del='no'");
}
...
if (form_isset("submitted")) {
	...
	if (sizeof($del_messages)) {
	    foreach ($del_messages as $msg_id):
			 $db1->doSql("UPDATE $tab[mail] SET del='yes' WHERE inbox='transfers' AND dest='$id' AND del='no' AND id='$msg_id'");
		endforeach;
	}
}
...
<form name="frmDelete" method="post" action="transfers.php?tru=<?= $tru ?>">
...
foreach ($rows as $sole):
          ...
          <table width="98%" height="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFFFFF">
           <tr bgcolor="#666666">
            ...
            &nbsp;<font color="#FF0000">&nbsp;<font size="2"><b>received from</b></font></font><font size="2"> 
            <?if($sole[src]==99999){?>
            <b><font color="#000000">SITE ALERT! </font>
            <?}else{?>
            <?if($crw[1]){?>
