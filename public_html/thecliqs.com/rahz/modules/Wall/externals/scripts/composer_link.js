/* $Id: composer_link.js 18.06.12 10:52 michael $ */



Wall.Composer.Plugin.Link = new Class({
  ...
  activate : function(no_focus) {
    ...
    this.elements.formInput = new Element('input', {
      'class' : 'wall-compose-form-input wall-compose-link-form-input',
      'type' : 'text'
    }).inject(this.elements.body);

    this.elements.formSubmit = new Element('button', {
      'class' : 'wall-compose-form-submit wall-compose-link-form-submit',
  ...
  doShowPreview : function() {
    ...
    if( this.params.loadedImages.length > 0 ) {
      ...
      this.elements.previewImages = new Element('div', {
        'class' : 'wall-compose-preview-images wall-compose-link-preview-images'
      }).inject(this.elements.body);

      this.params.assets.each(function(element, index) {
        if( !$type(this.params.loadedImages[index]) ) return;
        element.addClass('wall-compose-preview-image-invisible').inject(this.elements.previewImages);
        if( !this.checkImageValid(element) ) {
          delete this.params.images[index];
          delete this.params.loadedImages[index];
          element.destroy();
        } else {
          element.removeClass('wall-compose-preview-image-invisible').addClass('wall-compose-preview-image-hidden');
    ...
    this.elements.previewInfo = new Element('div', {
      'class' : 'wall-compose-preview-info wall-compose-link-preview-info'
    }).inject(this.elements.body);
    
    // Generate title and description
    this.elements.previewTitle = new Element('div', {
      'class' : 'wall-compose-preview-title wall-compose-link-preview-title'
    ...
    this.elements.previewDescription = new Element('div', {
      'class' : 'wall-compose-preview-description wall-compose-link-preview-description',
    ...
    if( this.params.loadedImages.length > 0 ) {
      this.elements.previewOptions = new Element('div', {
        'class' : 'wall-compose-preview-options wall-compose-link-preview-options'
      }).inject(this.elements.previewInfo);

      if( this.params.loadedImages.length > 1 ) {
        this.elements.previewChoose = new Element('div', {
          'class' : 'wall-compose-preview-options-choose wall-compose-link-preview-options-choose',
          'html' : '<span>' + this._lang('Choose Image:') + '</span>'
        }).inject(this.elements.previewOptions);

        this.elements.previewPrevious = new Element('a', {
          'class' : 'wall-compose-preview-options-previous wall-compose-link-preview-options-previous',
        ...
        this.elements.previewCount = new Element('span', {
          'class' : 'wall-compose-preview-options-count wall-compose-link-preview-options-count'
        }).inject(this.elements.previewChoose);


        this.elements.previewPrevious = new Element('a', {
          'class' : 'wall-compose-preview-options-next wall-compose-link-preview-options-next',
          'href' : 'javascript:void(0);',
      ...
      this.elements.previewNoImage = new Element('div', {
        'class' : 'wall-compose-preview-options-none wall-compose-link-preview-options-none'
      }).inject(this.elements.previewOptions);

      this.elements.previewNoImageInput = new Element('input', {
        'class' : 'wall-compose-preview-options-none-input wall-compose-link-preview-options-none-input',
        'type' : 'checkbox',
        'events' : {
      ...
      this.elements.previewNoImageLabel = new Element('label', {
        'for' : 'wall-compose-link-preview-options-none-input',
  ...
  setImageThumb : function(element) {
    ...
    if( this.elements.imageThumb ) {
      this.elements.imageThumb.addClass('wall-compose-preview-image-hidden');
    }
    if( element ) {
      element.removeClass('wall-compose-preview-image-hidden');
