...
function onload() {
    var url = '<?php echo $this->ad->url?>';
    if (url != '') {
        selectExternal();
        $('external_type').set('checked',true);
        $('external_url').set('value', '<?php echo $this->ad->url?>');
    }
    else {
        selectInternal();
        $('internal_type').set('checked',true);
        $('internal_module').set('value', '<?php echo $this->ad->module_id?>');
        updateItem();
        $('internal_item').set('value', '<?php echo $this->ad->item_id?>');
    }
    
    var ad_type = '<?php echo $this->ad->ad_type?>';
    var type_radio = $(ad_type+'_type');
    type_radio.set('checked',true);
    activeDiv(type_radio);
    var title = $('title');
    title.set('value', '<?php echo $this->ad->name?>');
    updateTitleLeft(title);
    var text = $('text');
    if (text) {
        text.set('value', '<?php echo $this->ad->description?>');
        updateTextLeft(text);
    }
    var benefit_amount = $('benefit_amount');
    if (benefit_amount) {
        benefit_amount.set('value', '<?php echo $this->ad->benefit_total?>');
        calculatePrice(benefit_amount);
    }
    var placements = $$('.placement');
    placements.each(function(placement) {
        if (placement.value == 'footer') updatePages(placement);
    });
    var campaign_id = $('campaign_id');
    campaign_id.set('value', '<?php echo $this->ad->campaign_id?>');
    updateCampaign(campaign_id);
    var start_date = '<?php echo ($this->ad->start_date) ? $this->ad->getStartDate()->get('yyyy-MM-dd HH:mm:ss') : ''?>';
    var end_date = '<?php echo ($this->ad->end_date) ? $this->ad->getEndDate()->get('yyyy-MM-dd HH:mm:ss'): ''?>';
    if (start_date == '') {
        $('continuously_schedule').set('checked', true);
        selectSchedule($('continuously_schedule'));
    }
    else {
    ...
    age_from.set('value', '<?php echo $this->target->age_from?>');
    var age_to = $('age_to');
    age_to.set('value', '<?php echo $this->target->age_to?>');
    var gender = $('gender');
    gender.set('value', '<?php echo $this->target->gender?>');
    var cities = $('cities');
    if (cities) cities.set('value', '<?php echo $this->target->cities?>');
    var interests = $('interests');
    if (interests) interests.set('value', '<?php echo $this->target->interests?>');
    var birthday = $('birthday');
    birthday.set('checked', <?php echo $this->target->birthday?>);
    var guest = $('public');
    guest.set('checked', <?php echo $this->target->public?>);
    var profile_type = $('profile_type_<?php echo $this->target->profile_type?>');
    profile_type.set('checked', true);
...
function placeOrder() {
    $('place_order').set('value', 1);
    checkValidate();
    $('place_order').set('value', 0);
}
</script>
<div id="error_div">
...
<form id="edit_ad" method="post" enctype="multipart/form-data">
    <div id="item_type" class="form_div">
        ...
        <div class="form_content">
            <div class="left_content">
                ...
                <input type="radio" name="item_type" id="external_type" value="external" onclick="selectExternal()"/>
    ...
    <div id="ad_type" class="form_div">
        ...
        <div class="form_content">
            ...
            <div class="ad_type_div">
                ...
                <label class="form_label" for="text_type"><?php echo ucwords($type)?></label>
    ...
    <div id="preview_and_edit" class="form_div">
        ...
        <div class="form_content">
                        ...
                        <div class="ynsocial_ads_content">
                            <div class="ynsocial_ads_item">
                                ...
                                <p class="ynsocial_ads_cont_image">
                                    <?php echo $this->itemPhoto($this->ad, 'thumb.normal'); ?>
                                </p>
            ...
            <?php if ($this->selectedPlacements) : ?>
            <?php $i = 0; ?>
            <?php foreach ($this->selectedPlacements as $placement) : ?>
            <div <?php if ($i == 0) echo 'id="original_placement"'?> class="sub_div placement_div">
                <?php if ($i != 0) : ?>
                <div class="remove_link">
                    <a href="" class="buttonlink remove_placement_icon" onclick="removePlacement(this, event)"><?php echo $this->translate('remove');?></a>
                </div>
                <?php endif; ?>
                <div class="left_content">
                    <div id="placement_div" class="form_element">
                        ...
                        <select id="placement" class="placement" name="placement[]" onchange="updatePages(this)">
                        <?php foreach ($this->placements as $key => $value) : ?>
                            <option value="<?php echo $key?>" <?php if ($key == $placement) echo "selected"?>><?php echo $value?></option>
                        <?php endforeach; ?>    
                        </select>
                    </div>
                    <?php $pages = $this->package->getAllPages($placement)?>
                    <div class="pages_div" class="form_element">
                        ...
                        <?php foreach ($pages as $page) : ?>
                            <option value="<?php echo $page['block_id']?>" <?php if (in_array($page['block_id'], $this->selectedAdblocks)) echo "selected"?>><?php echo $page['displayname']?></option>
                        <?php endforeach; ?>
                        </select>
        ...
        <div class="form_content">
            <div class="price_div">
                ...
                <p class="description">
                    ...
                    <span class="bold red"><?php echo ($this->package->price.' '.$this->package->currency) ?></span>
                    <span><?php echo $this->translate(' for ') ?></span>
                    <span class="bold"><?php echo ($this->package->benefit_amount.' '.ucwords($this->package->benefit_type).'s.')?></span>
                </p>
                <p class="description"><?php echo $this->translate('Please select your goal.')?></p>
                <input onchange="calculatePrice(this)" type="text" required id="benefit_amount" name="benefit_total"/>
                <label class="bold inline" for="benefit_amount"><?php echo ucwords($this->package->benefit_type).'s'?></label>
                <h3><p class="form_label" id="price_label"><?php echo $this->translate('Price')?></p></h3>
                <h2 id="price" class="bold red"><?php echo ($this->package->price.' '.$this->package->currency) ?></h2>
                <?php endif;?>
            </div>
        ...
        <div class="form_content">
            ...
            <div class="sub_div">
                ...
                <div class="content_right">
                    ...
                    <?php foreach ($this->countries as $key => $value) : ?>
                        <option value="<?php echo $key?>" <?php if (in_array($key, $this->target->countries)) echo "selected";?>><?php echo $value?></option>
                    <?php endforeach; ?>     
                    </select>
                    <p class="small_description"><?php echo $this->translate('Press Ctrl and click to select multiple types.')?></p>
                </div>
            ...
            <div class="sub_div">
                ...
                <div class="content_right">
                    ...
                    <?php foreach ($this->networks as $network) : ?>
                        <option value="<?php echo $network->getIdentity()?>" <?php if (in_array($network->getIdentity(), $this->target->networks)) echo "selected";?>><?php echo $network->getTitle()?></option>
                    <?php endforeach; ?>   
                    </select>
                    <p class="small_description"><?php echo $this->translate('Press Ctrl and click to select multiple types.')?></p>
                </div>
            ...
            <div class="sub_div">
                ...
                <div class="content_right">
                    ...
                    <input type="radio" name="profile_type" id="profile_type_0" value="0"/>
    ...
    <input type="hidden" id="place_order" name="place_order" value="0"/>
    <div class="buttons">
      <button type="button" onclick="checkValidate()"><?php echo $this->translate("Save")?></button>
      <button type="button" onclick="placeOrder()"><?php echo (($this->package->price != 0) ?  $this->translate("Save & Place Order") : $this->translate("Save & Publish"))?></button>
      <span> <?php echo $this->translate('or')?> </span>
      <span><?php echo $this->htmlLink(
            array('route' => 'ynsocialads_ads', 'module' => 'ynsocialads', 'controller' => 'ads', 'action' => 'view', 'id' => $this->ad->getIdentity()),
            $this->translate('Cancel'), array()) ?></span>
    </div>
</form>
<?php endif; ?>
<script type="text/javascript">
