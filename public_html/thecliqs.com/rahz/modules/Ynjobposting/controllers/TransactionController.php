<?php
class Ynjobposting_TransactionController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'ynjobposting') -> findRow($order_id);
			}
			else
			{
				$this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'ynjobposting_transaction', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> ynjobposting();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'ynjobposting') -> findRow($orderId)) 
	...
	public function finishAction()
	{
		...
		if ($status == 'completed')
		{
			if($order -> type == 'job')
			{
				$url = $this -> view -> escape($this -> view -> url(array('controller' => 'jobs', 'action'=>'view', 'id' => $order -> item_id), 'ynjobposting_extended', true));
			}
			else 
			{
				$url = $this -> view -> escape($this -> view -> url(array('controller' => 'company', 'action'=>'detail', 'id' => $order -> item_id), 'ynjobposting_extended', true));
			}
		}
		else
		{
			//TODO redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'ynjobposting_general', true));
