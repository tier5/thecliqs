...
var TimelineManager = new Class({
    ...
    initEvents:function () {
        ...
        if (self.options.getElementById('mp-options')) {
            self.options.getElementById('mp-options').addEvent('click', function () {
                self.moreOptions(this);
            });
        }
        if ($(self.content).getElementById('mw-options')) {
            $(self.content).getElementById('mw-options').addEvent('click', function () {
                self.moreOptions(this);
            });
        }

        if ($(self.content).getElementById('profile-rate')) {

            var div = $(self.content).getElementsByClassName('tl-profile-photo')[0];

            $(self.content).getElementById('profile-photo').addEvent('mouseenter', function () {
                $(self.content).getElementById('profile-rate').removeClass('timeline-rate-hidden');
            });
            $(self.content).getElementById('profile-photo').addEvent('mouseleave', function () {
                $(self.content).getElementById('profile-rate').addClass('timeline-rate-hidden');
            });
        }
