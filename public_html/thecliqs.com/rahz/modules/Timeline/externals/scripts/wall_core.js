/* $Id: Wall.js 2010-05-25 01:44 michael $ */
var Wall = {};
...
Wall.Slideshow = new Class({
    ...
    initialize:function (src, guid, element, $page_id) {
        var self = this;
        ...
        if (!is_popular_list) {
            ...
            if (!feed.checkActive) {
                ...
                var data = $merge(feed.params, {
                    ...
                    'mindate':this.options.last_date,
                    'checkUpdate':true
                });
...
Wall.Feed = new Class({
    ...
    initializeComposer:function () {
        ...
        var $tmp = this.feed.getElement('.wall-social-composer');
        if ($$('html[dir=rtl]').length) {
            $tmp.removeClass('l');
            $tmp.addClass('r');
        }
    ...
    setShareEnabled:function (provider, enabled, object_name) {
                ...
                if (provider == 'facebook') {
                    ...
                    if (service) {
                        if (service.options.fb_pages && service.options.fb_pages.length) {
                            var select_box = '<form action=""><div class="wall_select_page">' + en4.core.language.translate('WALL_CHOOSE_MY_PAGE') + '<div><input type="radio" name="wall_fbpage_choose_page" id="wall_fbpage_choose_page_no"/> <label for="wall_fbpage_choose_page_no">' + en4.core.language.translate('WALL_FBPAGE_NO') + '</label></div>';

                            service.options.fb_pages.each(function (item) {
                                select_box += '<div><input type="radio" name="wall_fbpage_choose_page" id="wall_fbpage_choose_page_' + item.fbpage_id + '" value="' + item.fbpage_id + '" ' + ((self.options.fbpage_id == item.fbpage_id) ? 'checked="checked"' : '') + '/>&nbsp; <label for="wall_fbpage_choose_page_' + item.fbpage_id + '">' + item.title + '</label></div>';
                            });
                            select_box += '</select></div></form>';
                            title += select_box;
                        }
                    }
                }
        ...
        var tips = Wall.elements.get('Wall.Tips', (window.$uid || Slick.uidOf)($share));
        if (tips) {
            tips.setTitle(title);
            tips.$inner.getElements('a.wall_logout').addEvent('click', function () {
                Wall.services.get(provider).logout();
            });
            tips.$inner.getElements('input[name=wall_fbpage_choose_page]').addEvent('change', function () {
    ...
    initActionMenu:function (action) {
        ...
        var $menu = Wall.injectAbsolute($link, action.getElement('.wall-menu'), true);
    ...
    initAction:function ($container, params) {
        ...
        $container.getElements('.utility-getlast .link').addEvent('click', function () {
            ...
            var data = $merge(self.params, {
                'minid':self.watcher.options.last_id,
    ...
    loadFeed:function (options, where, oncomplete, params, element, effect_item) {
        ...
        var request_fn = function (html) {
            ...
            if (where) {
                    ...
                    try {
                        if (!params.viewall) {
                            $container.getElements('.utility-setlast').each(function (item) {
                                ...
                                self.setLasts(last_date, last_id);
                            });
                        }
                        $container.getElements('.utility-setlast').destroy();
                ...
                var ranges = '';
                if (element && element != null && element.hasClass('sep')) {
