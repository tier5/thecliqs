...
// Process
    ...
    if (version_compare($coreItem['version'], '4.1.7') < 0) {
        ...
        $this->headScript()
            ->prependFile($this->baseUrl() . '/externals/mootools/mootools-more-1.4.0.1-full-compat-' . (APPLICATION_ENV == 'development' ? 'nc' : 'yc') . '.js')
            ->prependFile($this->baseUrl() . '/externals/mootools/mootools-core-1.4.5-full-compat-' . (APPLICATION_ENV == 'development' ? 'nc' : 'yc') . '.js');
    }
