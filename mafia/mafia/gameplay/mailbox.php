...
if (form_isset("submitted_all")) {
    mysql_query("UPDATE $tab[mail] SET del='yes' WHERE inbox='inbox' AND dest='$id' AND del='no'");
}
...
if (form_isset("submitted")) {
	...
	if (sizeof($del_messages)) {
	    foreach ($del_messages as $msg_id):
			 $db1->doSql("UPDATE $tab[mail] SET del='yes' WHERE inbox='inbox' AND dest='$id' AND del='no' AND id='$msg_id'");
		endforeach;
	}
}


$pimp = mysql_fetch_array(mysql_query("SELECT crew,status FROM $tab[pimp] WHERE id='$id';"));
...
?><body>
   <?
  if(($pimp[1] == admin) && ($poster))
      {
      ?>
   <br>
   <b><font color="red">
   <?=$poster?>
   's posting privlegdes have been 
   <?=$priv?>
   .</font><br>
   <?
      mysql_query("UPDATE $tab[pimp] SET postpriv='$priv' WHERE pimp='$poster' AND status!='admin'");
      }
	  ?><br>
   <table width="500">
     <tr>
     ...
     <tr>
       <td align="center" colspan="5"><b><font color="#ff0000" size="2">showing messages &nbsp;</font></b></td>
     </tr>
   </table>
   <br /><br /><br />
...
<form name="frmDelete" method="post" action="mailbox.php?tru=<?= $tru ?>">
...
foreach ($rows as $sole):
          $pmp = mysql_fetch_array(mysql_query("SELECT pimp,crew,status,postpriv FROM $tab[pimp] WHERE id='$sole[1]';"));
		  $pmppp = mysql_fetch_array(mysql_query("SELECT pimp,crew,status,postpriv FROM $tab[pimp] WHERE id='$id';"));
          $crw=mysql_fetch_array(mysql_query("SELECT id,icon FROM $tab[crew] WHERE id='$pmp[1]';"));
          ?>
          <table width="98%" height="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#ff0000">
           <tr bgcolor="#cccccc">
            <td align="left" valign="middle" bgcolor="cccccc"><input type="checkbox" name="delmsg<?= $sole['id'] ?>" />
              &nbsp;&nbsp;<font size="2"><b><font color="#FF0000">received from</font></b> 
              <font color="#FF0000">
              <?if($sole[src]==0){?>
              </font>            <b><font color="#000000"><strong>SITE ALERT</strong></font>
            <?}else{?>
            <?if($crw[1]){?>
            ...
            <?}?>
                ...
                <font color="#ff0000">
<?=countdown($sole[4])?>
ago"</font></font></b></small><font color="#ff0000">
            <?}?>
			</font></font></td>
            <td align="right" valign="middle" bgcolor="cccccc"><font size="2"><b>
            <?if($pmppp[2] == admin){?><a href="mailbox.php?poster=<?=$pmp[0]?>&priv=<?if($pmp[3] == enabled){echo"disabled";}else{echo"enabled";}?>&tru=<?=$tru?>"><?if($pmp[3] == enabled){echo"disabled";}else{echo"enabled";}?></a>  :: <?}?><?if($sole[src]!=99999){?>
              <a href="out.php?pid=<?=$pmp[0]?>&tru=<?=$tru?>&camefrom=<?=$pmppp[0]?>">reply</a> ::  <?}?><a href="godadmins.php?tru=<?=$tru?>">report</a></small></b></font></td>
           </tr>
           <tr bgcolor="#cccccc">
            <td colspan="2" bgcolor="#cccccc"><font color="#000000" size="2">
            <?if($pmp[2] == banned){?>
            <font color="#0000FF"><b>This member has been removed from the game.</b></font><br>
            <br>
            <?}elseif($pmp[3] == disabled){?>
            <font color="#0000FF"><b>This members posting privledges have been disabled.</b></font><br>
            <br>
            <?}else{?><?=securemsg($sole[3])?><?}?></font></td>
           </tr>
      </table>  
                   <br>
<br>
