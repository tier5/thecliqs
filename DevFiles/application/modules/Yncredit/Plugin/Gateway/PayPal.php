<?php
class Yncredit_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Yncredit_Model_Order $order, array $params = array())
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buying %s credits', (int)$order->credit),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buying %s credits', $order->credit),
  ...
  public function onPackageTransactionReturn(Yncredit_Model_Order $order, array $params = array())
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'yncredit') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'yncredit') -> findRow($info['PAYMENTREQUESTID']);
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Yncredit_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       }
