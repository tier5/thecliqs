			...
			switch (e.key) {
				case 'enter':
					...
					if( !this.selected || !this.visible) {
                      if( !this.options.customChoices ) {
                      	addNewChoice(this.element, this.options.toValues);
	...
	choiceSelect: function(choice) {
		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  {
                  	this.doAddValueToHidden(choice.id, token.id, this.options.toValues, true, null, token.url, token);
                  }
                }
                
	},
        
        doAddValueToHidden: function (name, toID, hideLoc, newItem, list, url, token){
          ...
          if (this.checkSpanExists(hideLoc, toID)){
            if (previousToValues==''){
            ...
            this.doPushSpan(name, toID, newItem, hideLoc, list, url, token);
          }
        },

        doPushSpan: function(name, toID, newItem, hideLoc, list, url, token){
          // summary:
          //		This is called to create a new span item with the id.
			
          var myElement = new Element("span");
          myElement.id = hideLoc+"_tospan_"+toID;
          switch (token.type) {
			case 'user':
				myElement.addClass("user_tag");
				myElement.innerHTML = "<a target='_blank' href="+url+">"+token.photo+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\", \""+this.element.get('id')+"\");'><i class='fa fa-times'></i></a>";
				break;
			case 'genre':
				myElement.addClass("genre_tag");
				myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\", \""+this.element.get('id')+"\");'><i class='fa fa-times'></i></a>";
				break;	
			case 'artist':
				myElement.addClass("artist_tag");
				myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\", \""+this.element.get('id')+"\");'><i class='fa fa-times'></i></a>";
				break;	
          }
          document.getElementById(this.options.toValues+'-wrapper').show();
          document.getElementById(this.options.toValues+'-wrapper').style.height= 'auto';


          if (list == null) list = "";

          document.getElementById(this.options.toValues+'-element').appendChild(myElement);
          this.fireEvent('push');
        },
        ...
        checkSpanExists: function (hideLoc, toID){
          var span_id = hideLoc+"_tospan_"+toID;
          if ($(span_id)){
            return false;
          }
          else return true;
...
var OverlayFix2 = new Class({
	...
	hide: function() {
		if (this.fix) this.fix.style.display = 'none';
		return this;
	},
...
Autocompleter2.Base = Autocompleter2;