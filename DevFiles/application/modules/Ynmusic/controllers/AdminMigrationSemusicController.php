<?php
class Ynmusic_AdminMigrationSemusicController extends Core_Controller_Action_Admin {
    public function init() {
        //get admin menu
        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
        	->getNavigation('ynmusic_admin_main', array(), 'ynmusic_admin_main_migrationsemusic');
		
		if (!Engine_Api::_()->hasModuleBootstrap('music')) {
    ...
    public function indexAction() {
		...
		$importedIds = Engine_Api::_()->getDbTable('imports', 'ynmusic')->getImportedIdsByType('music_playlist');
		$playlistTbl = Engine_Api::_()->getItemTable('music_playlist');
		$playlistTblName = $playlistTbl->info('name');
		$select = $playlistTbl->select()->from("$playlistTblName as object");
	...
	public function reportAction() {
		...
		$select -> where("report.from_type = 'music_playlist'");
		...
		if (!empty($params['ori_title'])) {
			$mPlaylistTbl = Engine_Api::_()->getItemTable('music_playlist');
			$mPlaylistTblName = $mPlaylistTbl->info('name');
			$select
				->joinLeft("$mPlaylistTblName as mPlaylist", "mPlaylist.playlist_id = report.from_id AND report.from_type = 'music_playlist'", "");
    ...
    public function multiremoveAction() {
        ...
        if ($this -> getRequest() -> isPost() && $confirm == TRUE) {
            ...
            $this -> _helper -> redirector -> gotoRoute(array('module'=>'ynmusic','controller'=>'migration-semusic', 'action'=>'report'), 'admin_default', TRUE);
        }
    }
}