...
class Ynresume_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynresume_Model_Node';
	 ...
	 * @param  $parent_id
	 * @return Ynresume_Model_Node|NULL
	 */
	 ...
	 * @param   Ynresume_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynresume_Model_Node
	 * @throw Exception
	 */
	public function insertAfter(Ynresume_Model_Node $node, $data) {
		if($node -> pleft == 1) {
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynresume_Model_Node $node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function appendChild(Ynresume_Model_Node $node, $data) {
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynresume_Model_Node $node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynresume_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynresume_Model_Node) {
