 ...
 * @author     Minh Nguyen
 */
class Ynauction_Widget_MostRatedYnauctionsController extends Engine_Content_Widget_Abstract
{
  ...
  if($this->_getParam('max') != ''){       
   ...
   $data = Ynauction_Model_Product::getProducts($where,'rates DESC',$limit);
   if(count($data) <= 0)
        $this->setNoRender();
   $this->view->data = $data;
