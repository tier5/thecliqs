<?php
class Ynbusinesspages_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Ynbusinesspages_Model_Order $order, array $params = array())
 	{
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Business'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Business from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buy Business'),
  ...
  public function onPackageTransactionReturn(Ynbusinesspages_Model_Order $order, array $params = array())
  {
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'ynbusinesspages') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'ynbusinesspages') -> findRow($info['PAYMENTREQUESTID']);
     ...
     try {
		...
		if($package_id)
		{
			Engine_Api::_() -> ynbusinesspages() -> buyBusiness($order -> item_id, $order -> package_id);
		...
		{
			Engine_Api::_() -> ynbusinesspages() -> featureBusiness($order -> item_id, $order -> feature_day_number);
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Ynbusinesspages_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       }
