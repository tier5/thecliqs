<?php
class Ynbusinesspages_TransactionController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'ynbusinesspages') -> findRow($order_id);
			}
			else
			{
				$this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'ynbusinesspages_transaction', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> ynbusinesspages();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'ynbusinesspages') -> findRow($orderId)) 
	...
	public function finishAction()
	{
		...
		$business = Engine_Api::_() -> getItem('ynbusinesspages_business', $order -> item_id);
		if ($status == 'completed')
		{
			$url = $this -> view -> escape($this -> view -> url(array('id' => $business -> getIdentity(), 'slug' => $business -> getSlug()), 'ynbusinesspages_profile', true));
		}
		else
		{
			//redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'ynbusinesspages_general', true));
