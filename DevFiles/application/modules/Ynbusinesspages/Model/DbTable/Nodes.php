...
class Ynbusinesspages_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynbusinesspages_Model_Node';
	...
	/**
	 * @return Ynbusinesspages_Model_Node
	 */
	 ...
	 * @param  $parent_id
	 * @return Ynbusinesspages_Model_Node|NULL
	 */
	 ...
	 * @param   Ynbusinesspages_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynbusinesspages_Model_Node
	 * @throw Exception
	 */
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynbusinesspages_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynbusinesspages_Model_Node$node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynbusinesspages_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynbusinesspages_Model_Node) {
