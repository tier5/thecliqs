<?php
class Yncontest_TransactionProcessController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'yncontest') -> findRow($order_id);
			}
			else
			{
				$this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'yncontest_transaction_process', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> yncontest();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'yncontest') -> findRow($orderId)) 
		...
		), 'yncontest_transaction_process', true);
	}

	public function finishAction()
	{
		$this -> view -> status = $status = $this -> _getParam('state');
		$order = Engine_Api::_() -> getItem('yncontest_order', $this->_getParam('order_id'));
		$contest = Engine_Api::_() -> getItem('yncontest_contest', $order -> item_id);
		if ($status == 'completed')
		{
			$url = $this -> view -> escape($this -> view -> url(array('action' => 'view', 'contestId' => $contest -> getIdentity(), 'slug' => $contest -> getSlug()), 'yncontest_mycontest', true));
		}
		else
		{
			//redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'yncontest_general', true));
