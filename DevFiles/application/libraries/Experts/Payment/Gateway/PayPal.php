 ...
 * @license    http://www.socialengine.net/license/
 */
class Experts_Payment_Gateway_PayPal extends Experts_Payment_Gateway
{
  // Support
  ...
  protected $_transactionMap = array(
    Experts_Payment_Transaction::REGION => 'LOCALECODE',
    Experts_Payment_Transaction::RETURN_URL => 'RETURNURL',
    Experts_Payment_Transaction::CANCEL_URL => 'CANCELURL',

    // Deprecated?
    Experts_Payment_Transaction::IPN_URL => 'NOTIFYURL',
    Experts_Payment_Transaction::VENDOR_ORDER_ID => 'INVNUM',
    Experts_Payment_Transaction::CURRENCY => 'CURRENCYCODE',
    Experts_Payment_Transaction::REGION => 'LOCALECODE',
  );
  ...
  public function getService()
  {
    if( null === $this->_service ) {
      $this->_service = new Experts_Service_PayPal(array_merge(
        $this->getConfig(),
        array(
  ...
  public function processIpn(Experts_Payment_Ipn $ipn)
  {
    ...
    if( !empty($rawData['test_ipn']) && !$this->getTestMode() ) {
      throw new Experts_Payment_Gateway_Exception('Test IPN sent in non-test mode');
    }
  ...
  public function test()
  {
    ...
    } catch( Experts_Service_PayPal_Exception $e ) {
