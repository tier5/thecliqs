...
if(parent_popup.hasClass('ynvideochannel-options-active')){parent_popup.removeClass('ynvideochannel-options-active');}})};var path='http://'+window.location.hostname;if(en4.core.baseUrl=="/")
path=path+en4.core.baseUrl+'application/modules/Advmenusystem/externals/images/';else
path=path+en4.core.baseUrl+'/application/modules/Advmenusystem/externals/images/';var jscolor={dir:'',bindClass:'color',binding:true,preloading:true,install:function(){jscolor.addEvent(window,'load',jscolor.init);},init:function(){if(jscolor.binding){jscolor.bind();}
...
if(valueElement.value=="transparent")
{styleElement.style.backgroundColor="#FFF";styleElement.style.color="#000";}
...
p.btn.onmousedown=function(){THIS.hidePicker();};p.btnS.style.lineHeight=p.btn.style.height;switch(modeID){case 0:var padImg=path+'hs.png';break;case 1:var padImg=path+'hv.png';break;}
p.padM.style.backgroundImage="url('"+path+"cross.gif')";p.padM.style.backgroundRepeat="no-repeat";p.sldM.style.backgroundImage="url('"+path+"arrow.gif')";p.sldM.style.backgroundRepeat="no-repeat";p.pad.style.backgroundImage="url('"+padImg+"')";p.pad.style.backgroundRepeat="no-repeat";p.pad.style.backgroundPosition="0 0";redrawPad();redrawSld();jscolor.picker.owner=THIS;document.getElementsByTagName('body')[0].appendChild(p.boxB);}
