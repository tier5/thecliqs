...
class Socialbridge_Api_Facebook extends Socialbridge_Api_Abstract
{
	...
	public function getOwnerInfo($params = array())
	{
		if ($this -> _objPlugin)
		{
			try
			{
				...
				$extrafields = '?fields=id,about,address,birthday,email,name,first_name,last_name,gender,picture,link,website';
	...
	public function getAlbums($params = array())
	{
		...
		{
			$cover = $this -> _objPlugin -> api('/'.$album['cover_photo']);
			$src_small = $src_thumb = $src_medium = $src_big = '';
			if($cover)
			{
				$covers = $cover['images'];
