 ...
 slider_w_Volume:  100, // pixels wide
  ...
  logPlay : function() {
    ...
    if( !this.tallied[song_id] ) {
      ...
      new Request.JSON({
        url: $$('head base[href]')[0].get('href') + 'mp3music/album/song-play-tally',     
        noCache: true,
        data: {
          format: 'json',
          song_id: song_id,
  ...
  _attachScrub : function() {
    ...
    if( this.container.getElement('.music_player_art').isDisplayed() ) {
      this.options.sliderWidth = chrome.width - img.width - 14;
    } else {
      this.options.sliderWidth = chrome.width - 14;
    }
