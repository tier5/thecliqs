<?php
class Ynauction_Model_Order extends Core_Model_Item_Abstract
{
	protected $_searchTriggers = false;
	protected $_type = 'ynauction_order';
	...
	public function getPackageParams($arr = array())
    {
        ...
        // General
        $params['name'] = $view -> translate('Buy Auction');
        $params['price'] = $arr['price'];
        $params['description'] = $view -> translate('Buy Auction from %s', $view -> layout() -> siteinfo['title']);
    ...
    {
        return 'ynauction_' . $user_id . '_fee_' . $fee;
    }
	...
	public function isChecked()
	{
		if ($this -> status != 'completed')
			return false;
		$table = Engine_Api::_() -> getDbTable('transactions', 'ynauction');
	...
	public function onPackageTransactionReturn(array $params = array())
	{
		...
		if (isset($params['state']) && $params['state'] == 'cancel')
		{
	     ...
	     $db->beginTransaction();
	     try {
			Engine_Api::_() -> ynauction() -> processPayment($this);
		    $db->commit();
	    } catch (Exception $e) {
	      $db->rollBack();
	      throw $e;
