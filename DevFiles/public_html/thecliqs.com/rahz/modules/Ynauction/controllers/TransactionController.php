<?php
class Ynauction_TransactionController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'ynauction') -> findRow($order_id);
			}
			else
			{
				return $this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'ynauction_transaction', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> ynauction();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'ynauction') -> findRow($orderId)) 
		...
		), 'ynauction_transaction', true);
	}

	public function finishAction()
	{
		$this -> view -> status = $status = $this -> _getParam('state');
		$order = Engine_Api::_() -> getItem('ynauction_order', $this->_getParam('order_id'));
		$product = Engine_Api::_() -> getItem('ynauction_product', $order -> item_id);
		if ($status == 'completed')
		{
			$url = $this -> view -> escape($this -> view -> url(array('action' => 'detail', 'auction' => $product -> getIdentity(), 'slug' => $product -> getSlug()), 'ynauction_general', true));
		}
		else
		{
			//redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'ynauction_general', true));
