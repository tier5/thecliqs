 ...
 function makeBill(f)
{
    ...
    if(f == null || f == undefined && is_already == false){     
      fr.submit();
         ...
         is_already = false;
         new Request.JSON({
          url: '<?php echo $this->url(array("module"=>"ynauction","controller"=>"proposal","action"=>"makebill"), "default") ?>',
          data: {
            'format': 'json',
            'proposal_id' : <?php echo $this->proposal->proposal_id; ?>
          },
...
<div class="table">    
    <table width="100%">
          ...
          <strong id="title"><?php echo $this->htmlLink($item->getHref(), $item->getTitle()) ?> </strong>
           ...
           <?php echo $this->locale()->toCurrency($this->proposal->proposal_price,$item->currency_symbol);?></font>
           </div>
           ...
           <div style="padding-top:5px;">
            ...
            <div class="p_4">
               ...
               <input TYPE="hidden" NAME="amount" VALUE="<?php echo $this->proposal->proposal_price;?>"/>
