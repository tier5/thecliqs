<?php
class Ynauction_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      ...
      $gateway = new $class(array(
        ...
        'currency' => Engine_Api::_() -> getApi('settings', 'core') -> getSetting('ynauction.currency', 'USD'),
      ));
  ...
  public function createPackageTransaction(Ynauction_Model_Order $order, array $params = array())
 	{
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Auction'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Auction from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buy Auction'),
  ...
  public function onPackageTransactionReturn(Ynauction_Model_Order $order, array $params = array())
  {
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'ynauction') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'ynauction') -> findRow($info['PAYMENTREQUESTID']);
     ...
     $db->beginTransaction();
     try {
     	Engine_Api::_() -> ynauction() -> processPayment($order);
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Ynauction_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try  again later.');
       }
