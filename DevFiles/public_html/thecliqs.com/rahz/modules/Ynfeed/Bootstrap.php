...
else
{
	...
	if(!$ra)
    {
        $query = "INSERT IGNORE INTO `engine4_younetcore_license` (`name`, `title`, `descriptions`, `type`, `current_version`, `lasted_version`, `is_active`, `date_active`, `params`, `download_link`, `demo_link`) VALUES ('ynfeed', 'Advanced Feed', 'Advanced Feed', 'module', '4.01p3', '4.01p3', '1', NULL, NULL, NULL, NULL);";
        $table->getAdapter()->query($query);
		$this->e($module_name,1);
    }
    else
    {
        $query = "Update `engine4_younetcore_license` set `lasted_version` = '4.01p3' , `current_version` = '4.01p3' where `name`='ynfeed' ";
        $table->getAdapter()->query($query);
