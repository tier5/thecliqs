	...
	choiceSelect: function(choice) {
		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.label, friend_ids, 'toValues', true, ' tag_friend', token.url);
                  ...
                  {
                  	this.doAddValueToHidden(choice.id, token.id, 'toValues', true, null, token.url);
                  }
                }
        ...
        doPushSpan: function(name, toID, newItem, hideLoc, list, url){
          ...
          if (newItem){
            ...
            myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
          }
          else{
            myElement.id = "tospan_"+name+"_"+toID;
            myElement.innerHTML = "<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
