...
var HESuggest = {
  ...
  share: function(html, app_id) {
    var cont = new Element('div', {'html':html, 'class':'share_container'});
    var cont2 = new Element('div', {'style':'position:fixed; z-index: 20; width: 0px;', 'id':'he_share_container'});


    if (Browser.Engine.trident5) {
      try {
        if (en4.core.subject.type == 'blog') {
          cont2.setProperties({'style':'position:fixed; z-index: 20; width: 0px;', 'id':'he_share_container'});
          cont.setStyle('left', '-290px');
        }
      } catch(e) {}
    }
    ...
    if (cont.getElement('.linkedin_share_box_container')) {
      var script = new Element('script', {'type':'IN/Share', 'data-counter':'top'});
      script.inject(cont.getElement('.linkedin_share_box_container'));
    }

    var script = new Element('script', {'src':'http://platform.twitter.com/widgets.js'});
    script.inject($('global_content'), 'before');

    var script = new Element('script', {'src':'http://platform.linkedin.com/in.js'});
    script.inject($('global_content'), 'before');

    var script = new Element('script', {'src':'https://apis.google.com/js/plusone.js', 'async':true});
    script.inject($('global_content'), 'before');
    ...
    window.addEvent('scroll', function(e) {
      var descripY = parseInt($('global_content').getOffsets().y) - 20;
      var scrollY = $(window).getScroll().y;
      var footerY = parseInt($('global_footer').getOffsets().y);
      var height = parseInt(cont2.getStyle('height'));
      var fixedShare = cont2.getStyle('position') == 'fixed';
