 <?php
	...
	$staticBaseUrl = $this->layout()->staticBaseUrl;
 	$this->headLink() ->prependStylesheet('//netdna.bootstrapcdn.com/bootstrap/3.0.0/css/bootstrap.min.css')
		->prependStylesheet($staticBaseUrl . 'application/modules/Yncontest/externals/styles/upload_photo/jquery.fileupload.css');
		
	$this->headScript()
  	->appendFile($staticBaseUrl . 'application/modules/Yncontest/externals/scripts/jquery-1.7.1.min.js')	
		->appendScript('jQuery.noConflict();')
  	->appendFile($staticBaseUrl . 'application/modules/Yncontest/externals/scripts/js/vendor/jquery.ui.widget.js')	
  	->appendFile($staticBaseUrl . 'application/modules/Yncontest/externals/scripts/js/jquery.iframe-transport.js')
		->appendFile($staticBaseUrl . 'application/modules/Yncontest/externals/scripts/js/jquery.fileupload.js')	
...
jQuery(function () 
{
    ...
    var count = 0;
    var url = '<?php echo $this->url(array('module' => 'yncontest', 'controller' => 'contest-photo', 'action' => 'upload-photo'), 'default')?>';
...
{
	new Request.JSON({
        url: '<?php echo $this->url(array('module'=>'yncontest','controller'=>'contest-photo','action'=>'remove'), 'default') ?>',
        data: {
          'format': 'json',
          'photo_id': photo_id
        }
