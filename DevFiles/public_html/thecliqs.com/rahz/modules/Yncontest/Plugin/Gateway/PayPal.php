<?php
class Yncontest_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Yncontest_Model_Order $order, array $params = array())
 	{
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Contest'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Contest from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buy Contest'),
  ...
  public function onPackageTransactionReturn(Yncontest_Model_Order $order, array $params = array())
  {
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'yncontest') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'yncontest') -> findRow($info['PAYMENTREQUESTID']);
     ...
     $db->beginTransaction();
     try {
     	Engine_Api::_() -> yncontest() -> processContest($order -> security_code, $info['TRANSACTIONID']);
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Yncontest_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       }
