...
class Ynmediaimporeter_Provider_Abstract
{
    ...
    /**
     * service identity name, etc: facebook, twiter, instagram, flickr, picasa, yfrog.
    ...
    public function getCurrentViewerId()
    {
        ...
        {
            $this -> _viewerId = Engine_Api::_() -> user() -> getViewer() -> getIdentity();
        }
    ...
    public function correctNodesStatus($nodes)
    {
        ...
        $select = $table -> select() -> from($table -> info('name'), array(
            'nid',
            'status'
        )) -> where("nid IN ($id)") -> where('user_id=?', intval(self::getViewerId()));

        foreach ($db -> fetchPairs($select) as $id => $status)
        {
            if (isset($nodes[$id]))
            {
