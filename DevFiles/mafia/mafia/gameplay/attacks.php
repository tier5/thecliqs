...
if (form_isset("submitted_all")) {
    mysql_query("UPDATE $tab[mail] SET del='yes' WHERE dest='$id'");
}
...
if (form_isset("submitted")) {
	...
	if (sizeof($del_messages)) {
	    foreach ($del_messages as $msg_id):
			 $db1->doSql("UPDATE $tab[mail] SET del='yes' WHERE id='$msg_id'");
		endforeach;
	}
}
...
$row = $db1->getRow("SELECT COUNT(id) as ct FROM $tab[mail] WHERE dest='$id' AND inbox='attacks' AND del='no'");
$count = $row['ct'];
...
<? if ($count > 0) { ?>
<form name="frmDelete" method="post" action="attacks.php?tru=<?= $tru ?>">
...
<?
#start pagination
$rows = $db1->createRecordset("SELECT id,src,dest,msg,time,del,crew FROM $tab[mail] WHERE dest='$id' AND inbox='attacks' AND del='no' ORDER BY time DESC");
          ...
          <table width="98%" height="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFFFFF">
           <tr bgcolor="">
            <td bgcolor="#000000">
             ...
             <tr>
              <td align="left" valign="middle" bgcolor="#cccccc"><input type="checkbox" name="delmsg<?= $sole['id'] ?>" />&nbsp;<font color="#FF0000">&nbsp;<font size="2">received from</font></font><font size="2"> 
                  <?if($crw[1]){?>
                  ...
                  <?=$pmp[0]?>
                  </a> <font color="#FFFFFF">"<b>
                <?=countdown($sole[4])?> 
                ago</b>"</font></font></td>
              <td align="right" valign="middle" bgcolor="#cccccc"><font size="2"><b><a href="hit.php?pid=<?=$pmp[0]?>&tru=<?=$tru?>">revenge</a> :: <a href="out.php?pid=<?=$pmp[0]?>&tru=<?=$tru?>&camefrom=attacks.php?tru=<?=$tru?>">message</a> :: <a href="pimpgods.php?tru=<?=$tru?>">report</a></small></b></font></td>
             </tr>
            </table>            </td>
           </tr>
           <tr>
            <td bgcolor="#cccccc" ><font color="#FFFFFF" size="2">
            <?=$sole[3]?>
            .</font></td>
           </tr>
      </table>
