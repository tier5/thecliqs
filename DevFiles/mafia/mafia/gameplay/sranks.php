...
$result = mysql_query("SELECT id,pimp,whore,thug,networth,online,city,rank,nrank,crew FROM $tab[pimp] WHERE id>0 AND status!='Supporter' ORDER BY nrank ASC");
 $rows_per_page = 30;
$screen = $_REQUEST['screen'];
$total_records = mysql_num_rows($result);
$pages = ceil($total_records / $rows_per_page);
GAMEHEADER("Supporter Ranks");
?>

<form method="post" action="franks.php?tru=<?=$tru?>">
<table width="98%" cellspacing="1" align="center">
  <div align="center"><br>
      <b><font size="+1">Supporter Ranks</font></b>
    <br>
    <br>
    <br>
  </div>
...
$sql .= "WHERE id>0 AND status='Supporter' ORDER BY nrank ASC LIMIT $start, $rows_per_page";$result = mysql_query($sql);
...
$online=$time-$row[5];
        ...
        if($id == $t10[0]){$rankcolor = "#5F0000";}
        elseif($rankstart==0){$rankcolor="#220000";$rankstart++;}
...
if ($screen > 0)
{
  ...
  $previous = $screen-1;
echo "<a href=\"sranks.php?tru=$tru&screen=$previous\">Previous</a>\n";
...
$url = "sranks.php?tru=$tru&screen=" . $i;
...
if ($screen < $pages)
{
  ...
  $next = $screen + 1;
echo "<a href=\"sranks.php?tru=$tru&screen=$next\">Next</a>\n";
