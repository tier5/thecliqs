...
if ((!fetch("SELECT tranid FROM $tab[paypal] WHERE tranid='$txn_id';")) && ($business == "$paypal_email_address"))

   {
    ...
    if($payment_gross == "5.00"){$turns="1";}
elseif($payment_gross == "15.00"){$turns="2";}
elseif($payment_gross == "30.00"){$turns="3";}
elseif($payment_gross == "45.00"){$turns="4";}
else{$turns="0";}
...
//update database to add turns to user in game they bought for
        ...
        mysql_query("UPDATE $tab[user] SET subscribe='supporter', status='supporter', statusexpire='$expires', subexpires='$sexpires' WHERE username='$custom'");
