...
var saveCurrentPageInfo = function(formElement) {
  var url = '<?php echo $this->url(array('action' => 'save', 'page' => $this->pageObject->getIdentity()), 'page_editor', true)?>';
...
var saveChanges = function()
{
  ...
  $$('.pages_content_buildable').each(function(element) {
    ...
    // Timelinne Page
    elData.is_timeline = '<?php echo $this->pageObject->is_timeline; ?>';
    // Timelinne Page
  ...
  var url = '<?php echo $this->url(array('action' => 'update', 'page' => $this->pageObject->getIdentity()), 'page_editor', true); ?>';
...
var openWidgetParamEdit = function(name, element) {
  ...
  var url = '<?php echo $this->url(array('action' => 'widget', 'page' => $this->pageObject->getIdentity()), 'page_editor', true)?>';
...
<h2><?php echo $this->translate('Page Layout Editor'); ?></h2>
<p>
  <?php echo $this->translate('PAGE_LAYOUT_DESC'); ?>
</p>
...
<div class="pages_layoutbox_menu">
	<ul>
		<li class="pages_layoutbox_menu_generic" id="pages_layoutbox_menu_openpage">
		  <?php if ($this->pageObject) : ?>
			<a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), $(this));">
         <span><?php echo $this->translate("Editing:"); ?>&nbsp;<?php echo $this->string()->truncate($this->pageObject->displayname, 17, '...') ?></span>
         <span class="more">&nbsp;</span>
       </a>
       <?php endif; ?>
			<div class="pages_layoutbox_menu_wrapper_generic pages_layoutbox_menu_pages_wrapper" id="pages_layoutbox_menu_pages_wrapper">
				<ul class="pages_layoutbox_menu_pages">
					<?php foreach( $this->pageList as $page ): ?>
            <li onclick="loadPage('<?php echo $this->url(array('page' => $page->page_id), 'page_editor', true);  ?>');">
              <?php echo $this->string()->truncate($page->displayname, 30, '...'); ?>
          	</li>
          <?php endforeach; ?>
				</ul>
			</div>
    ...
    <li id="pages_layoutbox_menu_createpage">
     ...
     <li id="pages_layoutbox_menu_viewpage">
      <?php echo $this->htmlLink( $this->url(array( 'page_id' => $this->pageObject->url ), 'page_view'), $this->translate('View Page') ); ?>
     </li>
     <li id="pages_layoutbox_menu_deletepage">
      <?php echo $this->htmlLink( $this->url(array( 'action' => 'delete', 'page_id' => $this->pageObject->page_id), 'page_team'), $this->translate('Delete Page') ); ?>
     </li>
     <li id="pages_layoutbox_menu_editpage">
      <?php echo $this->htmlLink( $this->url(array( 'action' => 'edit', 'page_id' => $this->pageObject->page_id), 'page_team'), $this->translate('Back To Edit Page Dashboard') ); ?>
     </li>
	</ul>
</div>
...
<?php if ($this->pageObject) : ?>
  ...
  <div class="pages_layoutbox_sub_menu">
    ...
    <?php if ($this->is_timeline_enabled) : // Timeline Page ?>
    <ul class="page-editor-timeline">

      <li onclick="<?php if($this->pageObject->is_timeline):?>changePageProfileType('<?php echo $this->url(array('action' => 'type', 'page' => $this->pageObject->getIdentity()), 'page_editor', true)?>');<?php endif;?>" class="<?php echo $this->default_class; ?>">
        <?php echo $this->translate('PAGE_EDITOR_PROFILE_Standart'); ?>
      </li>
      <li onclick="<?php if(!$this->pageObject->is_timeline):?>changePageProfileType('<?php echo $this->url(array('action' => 'type', 'page' => $this->pageObject->getIdentity()), 'page_editor', true)?>');<?php endif;?>" class="<?php echo $this->timeline_class; ?>">
        <?php echo $this->translate('PAGE_EDITOR_PROFILE_Timeline'); ?>
      </li>
    </ul>
    <?php if (!$this->timeline_converted): ?>
      <span id="page-editor-not-converted">
            <?php echo $this->translate('PAGE_EDITOR_PROFILE_Timeline not converted'); ?>
        </span>
      <?php endif; ?>
    <?php endif; // Timeline Page ?>
    <ul>
      <?php if( $this->pageObject->name !== 'header' && $this->pageObject->name !== 'footer' && $this->isAllowColsEdit): ?>
