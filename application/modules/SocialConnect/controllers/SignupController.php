  ...
  public function indexAction()
  {
    ...
    foreach( Engine_Api::_()->getDbtable('signup', 'user')->fetchAll() as $row ) {
      if( $row->enable == 1 ) 
      {
		 ...
		 $plugin =  new $class;
		 $form = $plugin -> getForm();
		 if($row -> class == 'User_Plugin_Signup_Account')
		 {
		 	$moduleTable = Engine_Api::_()-> getDbTable('modules', 'core');
			$select = $moduleTable -> select() -> where('name = ?', 'core') ->where('version >= ?', '4.8.6');
		 	if($moduleTable -> fetchRow($select))
			{
		 		$emailField = $form -> getEmailElementFieldName();
			}
			else 
			{ 
				$emailField = 'email';
			}
			$emailElement = $form -> getElement($emailField);
			if(isset($_SESSION['User_Plugin_Signup_Account']['data']['email']))
			{
