...
$staticBaseUrl = $this->layout()->staticBaseUrl;
	...
	$coverFile = Engine_Api::_()->getDbtable('files', 'storage')->find($song->cover_id)->current();
	$coverPhotoUrl = $this->baseUrl().'/application/modules/Ynmusic/externals/images/song_cover_default.png';
	$hasCover = false;
	if($coverFile){
	...
	if (!$photoUrl)	 $photoUrl = $this->baseUrl().'/application/modules/Ynmusic/externals/images/nophoto_song_thumb_profile.png';	
?>

<div id="music-detail-<?php echo $song->getGuid()?>" rel="<?php echo $song->getGuid()?>" class="music-item music-detail song-cover music-profile-cover" style="background-image: url('<?php echo $coverPhotoUrl?>')">

	<img src="<?php echo $coverPhotoUrl ?>" alt="" class="song-bg-profile-cover music-bg-profile-cover" style="<?php echo 'top: '.$song->cover_top.'px'?>" />
	<img src="<?php echo $coverPhotoUrl ?>" alt="" class="song-bg-profile-cover music-bg-profile-cover reposition-cover" style="<?php echo 'top: '.$song->cover_top.'px'?>" />
	...
	<div class="reposition-cover-btn" <?php if (!$hasCover) echo 'style="display:none;"'?>>
		...
		<div class="reposition-cover-buttons" style="display: none;">
            ...
            <input class="cover-position" name="pos" value="<?php echo $song->cover_top?>" type="hidden">
        </div>
	</div>
	<?php endif;?>

	<div class="song-box-profile-info music-box-profile-info">
		<div id="song-title" class="music-profile-title">
			<?php echo $song;?>
		</div>
		
		<?php $album = $song->getAlbum();?>
		<?php if ($album) :?>
		<div id="song-album" class="music-profile-album">
			<?php echo $this->translate('In album: %s', $album);?>
		</div>
		<?php endif;?>
		
		<div class="box-time-image-song">
			<div class="time-song">
				<div class="duration-time"><?php echo date('i:s', $song->duration)?></div>
			</div>
			<?php 
				$noPlayImg = $song->getNoPlayImage();
				$playImg = $song->getPlayImage();
			?>
		...
		<div class="music-info-bottom">
			<div class="play-btn-<?php echo $song->getGuid()?> music-play-btn parent">
	...
	<div id="song-photo" class="music-profile-photo">
		...
		<div id="song-info" class="music-songs-info clearfix">
			<div class="song-duration music-duration">
				<?php echo date('i:s', $song->duration)?>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
var cover_top = <?php echo $song->cover_top?>;
...
function saveReposition() {
    if (jQuery('input.cover-position').length == 1) {
        ...
        new Request.JSON({
            'url': '<?php echo $this->url(array('action'=>'reposition', 'subject'=>$song->getGuid()),'ynmusic_general', true)?>',
            'method': 'post',
            'data' : {
                'position' : posY
            },
