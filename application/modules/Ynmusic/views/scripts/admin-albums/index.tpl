...
function featureAlbum(obj, id) {
    var value = (obj.checked) ? 1 : 0;
    var url = en4.core.baseUrl+'admin/ynmusic/albums/feature';
    new Request.JSON({
        url: url,
        method: 'post',
        data: {
...
function featureAll() {
    ...
    for (i = 1; i < inputs.length; i++) {
        if (!inputs[i].disabled && inputs[i].hasClass('featured_checkbox')) {
            ...
            featureAlbum(inputs[i], inputs[i].getProperty('data_id'));
        }
    }
}
...
function selectAll() {
    ...
    var inputs = multidelete_form.getElements('input.checkbox');
    for (i = 1; i < inputs.length; i++) {
...
function deleteSelected(){
    ...
    $('multidelete').action = en4.core.baseUrl +'admin/ynmusic/albums/multidelete';
    ...
    <table class='admin_table'>
        <thead>
            <tr>
                ...
                <th><a href="javascript:void(0);" onclick="javascript:changeOrder('album.title', 'ASC');"><?php echo $this->translate("Name") ?></th>
                ...
                <th><?php echo $this->translate("Genres") ?></th>
                <th><?php echo $this->translate("No of Songs") ?></th>
                <th><?php echo $this->translate("Lasted Update") ?></th>
                <th><input id="check_all_feature" onclick='featureAll();' type='checkbox' /><?php echo $this->translate("Featured") ?></th>
        ...
        <?php foreach ($this->paginator as $item): ?>
            <tr>
	            ...
	            <td><?php echo $item -> getCountSongs() ;?></td>
	            <td><?php echo $this->locale()->toDateTime($item->modified_date) ?></td>
	            <td><input data_id = '<?php echo $item->getIdentity()?>' type="checkbox" class="featured_checkbox" value="1" onclick="featureAlbum(this, '<?php echo $item->getIdentity()?>')" <?php if ($item->featured) echo 'checked'?>/></td>
	            <td class="option-link">
            	<?php if ($item->isEditable()) :?>
            		<a href="<?php echo $this->url(array('action' => 'edit', 'album_id' => $item -> getIdentity()), 'ynmusic_album', true) ?>"><?php echo $this->translate('Edit') ?></a>
	            <?php endif;?>
	            <?php if ($item->isDeletable()) :?>
            		<a class="smoothbox" href="<?php echo $this->url(array('action' => 'delete', 'id' => $item -> getIdentity()), 'ynmusic_album', true) ?>"><?php echo $this->translate('Delete') ?></a>
	            <?php endif;?>
	            </td>
                </td>
            </tr>
