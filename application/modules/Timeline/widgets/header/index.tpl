...
<?php
$this->headScript()
  ->appendFile($this->baseUrl() . '/application/modules/Timeline/externals/scripts/manager.js')
...
<div id="tl-navigation" class="tl-block">
  ...
  <div class="items tl-options">
    <ul class="main tl-in-block">
      ...
      <?php for ($i = 0; $i < $this->menuitems; $i++) : if (!isset($this->widgets[$i]) || !isset($this->widgets[$i]->content_id)) continue; ?>
      <li>
        <a href="javascript://"
           onclick="tl_manager.fireTab('<?php echo $this->widgets[$i]->content_id; ?>')"
           class="tab_button_<?php echo $this->widgets[$i]->content_id?>">
          <?php //echo $this->translate($this->widget_params[$this->widgets[$i]->name]['title']); ?>
          <?php echo ($this->widget_params[$this->widgets[$i]->name]['title']); ?>
          <?php if (array_key_exists('count', $this->widget_params[$this->widgets[$i]->name])): ?>
          <?php echo '(' . $this->widget_params[$this->widgets[$i]->name]['count'] . ')'; ?>
          <?php endif; ?>
        </a>
      </li>
      <?php endfor; ?>
      <li class="more <?php if (count($this->widgets) <= $this->menuitems): ?> hidden <?php endif; ?>">
        <a id='mw-options' href="javascript:void(0);"><?php echo $this->translate('more'); ?></a>
        <ul class="tl-in-block click-listener bound-mw-options">
          <?php for ($i = $this->menuitems; $i < count($this->widgets); $i++): if (!$this->widgets[$i]->content_id) continue; ?>
          <li>
            <a href="javascript://"
               onclick="tl_manager.fireTab('<?php echo $this->widgets[$i]->content_id; ?>')"
               class="tab_button_<?php echo $this->widgets[$i]->content_id?>">
...
<?php echo $this->content()->renderWidget('timeline.cover'); ?>
      ...
      <?php if ($this->profile_navigation && $this->profile_navigation->count() > 0): $navigation = $this->profile_navigation->toArray(); ?>
        <ul class="main tl-in-block">
          ...
          <li class="more <?php if (count($navigation) <= 2): ?> hidden <?php endif; ?>">
            <a id='mp-options' href='javascript://;'><?php echo $this->translate('more'); ?></a>
...
<div class="additional-row" id="additional-row">
  ...
  <div class="about">
    ...
    <?php echo $this->htmlLink('javascript://', $this->translate('Personal Information'), array('title' => $this->subject()->getTitle())); ?>
  </div>
  ...
  <div class="applications">
    ...
    <?php $i = 0;
    //print_die(array_keys($this->activeApplications));
            ...
            <div class="photo <?php if (!($thumb && $thumb->photo_id)) echo str_replace('.', '-', $widget->name); ?>">
              <?php if ($thumb && $thumb->photo_id):?>
                ...
                <?php if (array_key_exists($widget->name, $this->activeApplications)): ?>
                  <?php if (array_key_exists('items', $this->activeApplications[$widget->name]) &&
                    (is_array($this->activeApplications[$widget->name]['items']) || $this->activeApplications[$widget->name]['items'] instanceof Zend_Paginator)
                  ): $j = 0; ?>
