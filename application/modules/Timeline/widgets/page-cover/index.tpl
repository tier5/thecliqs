...
<?php
$this->headScript()
  ->appendFile($this->baseUrl() . '/application/modules/Timeline/externals/scripts/page_timeline/page_cover.js');
?>
...
<script type="text/javascript">
  document.tl_cover.setOptions({
    ...
    'is_allowed':true,
    'cover_url':'<?php echo $this->url(array('action' => 'get', 'id' => $this->subject()->getIdentity()), 'timeline_page_photo', true); ?>',
    'position_url':'<?php echo $this->url(array('action' => 'position', 'id' => $this->subject()->getIdentity()), 'timeline_page_photo', true); ?>'
  });
    ...
    <div id='tl-cover'>
      <a href="javascript:void(0);"
        <?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting('wall.content.slideshow', true) && $this->albumPhoto): ?>
         onclick="tl_cover.slideShow('<?php echo $this->albumPhoto->getPhotoUrl(); ?>', '<?php echo $this->albumPhoto->getGuid(); ?>', this, '<?php echo $this->subject()->getIdentity(); ?>')"
        <?php endif; ?>
        >
