...
<?php if($this->notView):  ?>
  <div class="tip">
      <span>
        <?php echo $this->translate('You can not buy this auction, please check detail auction page.');?>
      </span>
    </div>
  <?php
  else:
 ...
 function makeBill(f)
{
    ...
    if(f == null || f == undefined && is_already == false){     
      fr.submit();
         ...
         is_already = false;
         new Request.JSON({
          url: '<?php echo $this->url(array("module"=>"ynauction","controller"=>"index","action"=>"makebillbuynow"), "default") ?>',
          data: {
            'format': 'json',
...
<div class="table">    
    <div class="yntable noborder ynaution_display_index clearfix">
        ...
        <div class="ynaution_display_index_detail">
           ...
           <span style="font-weight: bold;"> <?php echo $this->translate('Price: ');?></span> <font color="red" style="font-weight: bold;"><?php echo $this->locale()->toCurrency($item->price,$item->currency_symbol);?></font>
           </div>
