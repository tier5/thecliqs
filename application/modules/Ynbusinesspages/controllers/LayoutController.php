...
class Ynbusinesspages_LayoutController extends Core_Controller_Action_Standard {
    ...
    public function init() {
        $this->_page_name = $page_name = $this->_request->getParam('page', 'ynbusinesspages_profile_index');
        $api = $this->getApi();
        ...
        $this->view->subjectType = $this->_subjectType = 'ynbusinesspages_business';
        $this->view->subjectId = $this->_subjectId = $business_id;
        $viewer = Engine_Api::_()->user()->getViewer();
		
        if (!$business->isAllowed('manage_page'))
		...
		{
			if(!$package -> allow_owner_manage_page)
			{
				return $this->_helper->requireAuth()->forward();
			}
		}
    ...
    /**
     * @return Ynbusinesspages_Api_Core
     */
    public function getApi() {
        if ($this->_api === NULL) {
            $this->_api = Engine_Api::_()->getApi('Layout', 'Ynbusinesspages');
        }
        return $this->_api;
    }
