 ...
 * @version    $Id: PayPal.php 2012-03-14 23:06:28Z mt.uulu $
 ...
 * @license    http://www.hire-experts.com
 */
class Experts_Service_PayPal extends Zend_Service_Abstract
{
  /**
  ...
  public function setOptions(array $options)
  {
    ...
    if( empty($this->_username) || empty($this->_password) ||
      ...
      throw new Experts_Service_PayPal_Exception('Not all connection ' .
  ...
  protected function _processHttpResponse(Zend_Http_Response $response)
  {
    ...
    if( 200 !== $response->getStatus() ) {
      throw new Experts_Service_PayPal_Exception(sprintf('HTTP Client ' .
    ...
    if( !is_string($responseStr) || '' === $responseStr ) {
      throw new Experts_Service_PayPal_Exception('HTTP Client returned an ' .
    ...
    if( !is_array($responseData) ||
      ...
      throw new Experts_Service_PayPal_Exception('HTTP Client returned ' .
    ...
    if( strtolower($responseData['ACK']) == 'failure' ) {
      ...
      throw new Experts_Service_PayPal_Exception(sprintf('API Error: ' .
      ...
      $supportedParams = null)
  {
    ...
    if( !is_array($params) ) {
      if( !empty($params) ) {
        throw new Experts_Service_PayPal_Exception('Invalid data type',
            'UNKNOWN_PARAM');
      } else {
        $params = array();
      }
    ...
    if( !empty($params) ) {
      ...
      throw new Experts_Service_PayPal_Exception(sprintf('Unknown param(s): ' .
    ...
    if( count($missingRequiredParams = array_diff_key($requiredParams, $foundKeys)) > 0 ) {
      ...
      throw new Experts_Service_PayPal_Exception(sprintf('Missing required ' .
