 ...
 * @license    http://www.socialengine.net/license/
 */
class Experts_Payment_Gateway_2Checkout extends Experts_Payment_Gateway
{
  // Support
   ...
   * Get the service API
   *
   * @return Experts_Service_2Checkout
   */
  public function getService()
  {
    if( null === $this->_service ) {
      $this->_service = new Experts_Service_2Checkout(array_merge(
        $this->getConfig(),
        array(
...
//    print_arr('given hash: ' . $givenHash);
//    print_arr('expected hash: ' . $expectedHash);
//    print_arr('vendor_id: ' . $rawData['vendor_id']);
//    print_arr('sale_id: ' . $rawData['sale_id']);
//    print_arr('vendor_order_id: ' . $rawData['vendor_order_id']);
//    print_die('invoice_id: ' . $rawData['invoice_id']);
  ...
  public function processTransaction(Experts_Payment_Transaction $transaction)
  {
    ...
    if( !isset($rawData['quantity']) || !is_numeric($rawData['quantity']) || $rawData['quantity'] <= 0 ) {
      ...
      if( isset($data['quantity']) && $data['quantity'] > 99 ) {
        throw new Experts_Payment_Gateway_Exception('Quantity must be less than or equal to 99.');
      }
...
//    }
    ...
    if( isset($rawData['merchant_order_id']) ) {
      if( strlen($rawData['merchant_order_id']) > 50 ) {
        throw new Experts_Payment_Gateway_Exception('Merchant Order ID cannot be longer than 50 character.');
      }
