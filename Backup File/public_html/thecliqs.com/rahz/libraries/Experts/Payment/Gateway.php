 ...
 * @license    http://www.socialengine.net/license/
 */
abstract class Experts_Payment_Gateway
{
  // General
  ...
  public function setConfig(array $config)
  {
    ...
    //  throw new Experts_Payment_Gateway_Exception('Missing username or password');
    //}
    $this->_config = $config;
  ...
  public function getLog()
  {
    if( null === $this->_log ) {
      if( !defined('APPLICATION_PATH') ) {
        ...
        $writer = new Zend_Log_Writer_Stream(APPLICATION_PATH . '/temporary/log/store.log');
  ...
  abstract public function processIpn(Experts_Payment_Ipn $ipn);

  abstract public function processTransaction(Experts_Payment_Transaction $transaction);
   ...
   * Test the login info
   *
   * @throws Experts_Payment_Gateway_Exception on failure
   */
  ...
  protected function _translateTransactionData($params, &$missingParams = null)
  {
    ...
    foreach( $params as $key => $value ) {
      if( isset($this->_transactionMap[$key]) ) {
        // Special cases
        switch( $key ) {
          case Experts_Payment_Transaction::REGION:
            $value = $this->isSupportedRegion($value);
            if( !$value ) {
              continue;
            }
            break;
          case Experts_Payment_Transaction::LANGUAGE:
            $value = $this->isSupportedLanguage($value);
            if( !$value ) {
              continue;
            }
            break;
          case Experts_Payment_Transaction::CURRENCY:
