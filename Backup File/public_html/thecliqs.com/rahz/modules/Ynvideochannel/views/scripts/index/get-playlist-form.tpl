...
<?php
	...
	$viewer = $this -> viewer();
	$playlistViewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('ynvideochannel_playlist', $viewer, 'auth_view');
    $playlistCommentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('ynvideochannel_playlist', $viewer, 'auth_comment');
	...
	window.addEvent('domready', function(){
		if($('play_list_cancel')) {
			$('play_list_cancel').addEvent('click', function (){
				...
				parent.innerHTML = "";
				parent.hide();
				parent.removeClass('ynvideochannel_active_add_playlist');
				return false;
			});
		}
		...
		if($('play_list_create'))
		{
			$('play_list_create').addEvent('click', function(){
				...
				var url = '<?php echo $this->url(array('action'=>'ajax-create-playlist'), 'ynvideochannel_general')?>';
		        new Request.JSON({
		            'url': url,
		            'method': 'post',
		            'data' : {
		            ...
		            'onSuccess': function(responseJSON) {
		                if(responseJSON.json == 'true') {
		                	var span = $$('.play_list_span.ynvideochannel_active_add_playlist')[0];
				            ...
				            (function() {
				            	notice.fade('out').get('tween').chain(function() {
				            		notice.destroy();
				            		span.removeClass('ynvideochannel_active_add_playlist');
				            		span.hide();
				            	});
