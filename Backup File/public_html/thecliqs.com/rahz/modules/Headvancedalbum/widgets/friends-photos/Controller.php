...
class Headvancedalbum_Widget_FriendsPhotosController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    $select = $table->select()
      ...
      ->where('p.photo_id NOT IN (?)', $prepare_hide_ids)
      ->group('p.photo_id')
      ->order('RAND()')
      ->limit($per_page+1); // take one more to check the end

    // Privacy
    // everyone, registered, owner_network, owner_member_member, owner_member
