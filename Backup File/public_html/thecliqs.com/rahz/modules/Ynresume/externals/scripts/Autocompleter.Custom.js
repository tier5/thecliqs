		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.label, friend_ids, this.options.toElementName, true, ' tag_friend', token.url);
                    // code to expend out the suggestion list
                    /*
                    for (var id in token.friends) {
                      this.doAddValueToHidden(token.friends[id], id, this.options.toElementName, true);
                    }*/
                  }
                  else 
                  {
                  	this.doAddValueToHidden(choice.id, token.id, this.options.toElementName, true, null, token.url);
                  }
                }
                
	},
        
        doAddValueToHidden: function (name, toID, hideLoc, newItem, list, url){
          ...
          if (this.checkSpanExists(name, toID)){
            ...
            this.doPushSpan(name, toID, newItem, hideLoc, list, url);
          }
        },

        doPushSpan: function(name, toID, newItem, hideLoc, list, url){
          // summary:
          ...
          document.getElementById(this.options.toElementName + '-wrapper').style.height= 'auto';


          if (list == null) list = "";
          myElement.addClass("tag"+list);

          document.getElementById(this.options.toElementName + '-element').appendChild(myElement);
          this.fireEvent('push');
        },
