		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.photo, token.label, friend_ids, 'toValues', true, ' tag_friend', token.url);
                  ...
                  {
                  	this.doAddValueToHidden(token.photo, choice.id, token.id, 'toValues', true, null, token.url);
                  }
                }
                
	},
        
        doAddValueToHidden: function (photo, name, toID, hideLoc, newItem, list, url){
          ...
          if (this.checkSpanExists(name, toID)){
            ...
            this.doPushSpan(photo, name, toID, newItem, hideLoc, list, url);
          }
        },

        doPushSpan: function(photo, name, toID, newItem, hideLoc, list, url){
          // summary:
          ...
          if (newItem){
            ...
            myElement.innerHTML = photo+"<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
          }
          else{
            myElement.id = "tospan_"+name+"_"+toID;
            myElement.innerHTML = photo+"<a target='_blank' href="+url+">"+name+"</a>"+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
