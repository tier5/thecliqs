...
class Ynultimatevideo_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynultimatevideo_Model_Node';
	...
	/**
	 * @return Ynultimatevideo_Model_Node
	 */
	 ...
	 * @param  $parent_id
	 * @return Ynultimatevideo_Model_Node|NULL
	 */
	 ...
	 * @param   Ynultimatevideo_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynultimatevideo_Model_Node
	 * @throw Exception
	 */
	public function insertAfter(Ynultimatevideo_Model_Node $node, $data) {
		if($node -> pleft == 1) {
		...
		try {
			...
			$newNode -> save();
			$this -> updateTree();
			$optionId = Engine_Api::_()->getApi('core', 'ynultimatevideo')->typeCreate($newNode->title);
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynultimatevideo_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function appendChild(Ynultimatevideo_Model_Node $node, $data) {
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynultimatevideo_Model_Node$node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynultimatevideo_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynultimatevideo_Model_Node) {
