...
class Ynlistings_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynlistings_Model_Node';
	 ...
	 * @param  $parent_id
	 * @return Ynlistings_Model_Node|NULL
	 */
	 ...
	 * @param   Ynlistings_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynlistings_Model_Node
	 * @throw Exception
	 */
	public function insertAfter(Ynlistings_Model_Node $node, $data) {
		if($node -> pleft == 1) {
		...
		try {
			...
			$this -> updateTree();
			$optionId = Engine_Api::_()->getApi('core', 'ynlistings')->typeCreate($newNode->title);
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynlistings_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function appendChild(Ynlistings_Model_Node $node, $data) {
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynlistings_Model_Node$node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynlistings_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynlistings_Model_Node) {
