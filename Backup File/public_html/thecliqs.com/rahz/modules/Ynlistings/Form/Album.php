<?php
class Ynlistings_Form_Album extends Engine_Form
{
	...
	public function init()
	{
		...
		$my_albums = Engine_Api::_() -> ynlistings() -> getUserAlbums(Engine_Api::_() -> user() -> getViewer());
		$album_options = Array();
		...
		if($this -> _addPhoto)
		{
			$album = Engine_Api::_() -> getItem('ynlistings_album', $this -> _albumId);
	...
	{
		$this -> getElement('ynlistings_album') -> setValue(0);
	}
	...
	public function saveValues($arr_photo_id = array())
	{
		...
		else
		{
			if (!isset($album))
			{
				$album = Engine_Api::_() -> getItem('ynlistings_album', $values['album']);
			}
		}
