<?php
class Ynlistings_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Ynlistings_Model_Order $order, array $params = array())
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Publish Listing'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Publish Listing from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Publish Listing'),
  ...
  public function onPackageTransactionReturn(Ynlistings_Model_Order $order, array $params = array())
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'ynlistings') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'ynlistings') -> findRow($info['PAYMENTREQUESTID']);
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Ynlistings_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       }
