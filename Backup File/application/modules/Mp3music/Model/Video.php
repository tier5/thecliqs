<?php
class Mp3music_Model_Video extends Core_Model_Item_Abstract
{
  ...
  public function getRichContent($view = false, $params = array())
  {
    ...
    if ($this->type ==3){
      $video_location = Engine_Api::_()->storage()->get($this->video_id, $this->getType())->getHref();
    ...
    $description = "<div class='video_desc'>".(Engine_String::strlen($tmpBody) > 368 ? Engine_String::substr($tmpBody, 0, 368) . '...' : $tmpBody)."</div>";
  ...
  public function compileYouTube($code, $view){
    //560 x 340
    $embedded = '
    <object width="'.($view?"368":"368").'" height="'.($view?"230":"230").'">
    ...
    <embed src="http://www.youtube.com/v/'.$code.'&color1=0xb1b1b1&color2=0xcfcfcf&hl=en_US&feature=player_embedded&fs=1'.($view?"":"&autoplay=1").'" type="application/x-shockwave-flash" allowfullscreen="true" allowScriptAccess="always" width="'.($view?"368":"368").'" height="'.($view?"280":"280").'"/>
    </object>';
    return $embedded;
  }
  public function compileVimeo($code, $view){
    //640 x 360
    $embedded = '
    <object width="'.($view?"368":"368").'" height="'.($view?"280":"280").'">
    <param name="allowfullscreen" value="true"/>
    <param name="allowscriptaccess" value="always"/>
    ...
    <embed src="http://vimeo.com/moogaloop.swf?clip_id='.$code.'&amp;server=vimeo.com&amp;show_title=1&amp;show_byline=1&amp;show_portrait=0&amp;color=&amp;fullscreen=1'.($view?"":"&amp;autoplay=1").'" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="'.($view?"368":"368").'" height="'.($view?"280":"280").'"/>
    </object>';
    return $embedded;
  }
  public function compileFlowPlayer($location, $view){
    ...
    en4.core.runonce.add(function(){\$('video_thumb_".$this->video_id."').removeEvents('click').addEvent('click', function(){flashembed('video_object_$this->video_id',{src: '".Zend_Registry::get('Zend_View')->baseUrl()."/externals/flowplayer/flowplayer-3.1.5.swf', width: ".($view?"368":"368").", height: ".($view?"280":"280").", wmode: 'transparent'},{config: {clip: {url: '$location',autoPlay: ".($view?"false":"true").", duration: '$this->duration', autoBuffering: true},plugins: {controls: {background: '#000000',bufferColor: '#333333',progressColor: '#444444',buttonColor: '#444444',buttonOverColor: '#666666'}},canvas: {backgroundColor:'#000000'}}});})});
    </script>";
    return $embedded;
  }
}