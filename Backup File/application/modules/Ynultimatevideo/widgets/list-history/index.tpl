<div class="ynultimatevideo_count_videos">
    ...
    <?php
        ...
        echo $this->translate(array('%1$s item', '%1$s items', $totalItem), $this->locale()->toNumber($totalItem));
    ?>
</div>

<?php if ($this->paginator->getTotalItemCount() > 0): ?>
	<div class="ynultimatevideo_list_my_videos ynultimatevideo_list-view">
        <div class="ynultimatevideo-playlist-listing">
            <ul class="ynultimatevideo_list_most_items clearfix">
                <?php foreach ($this->paginator as $history): ?>
                    <?php $item = Engine_Api::_()->getItem($history['item_type'], $history['item_id']); ?>
                    <?php if ($item): ?>
                        <?php if ($item->getType() == 'ynultimatevideo_video'): ?>
                            <li class="ynultimatevideo_list_most_item ynultimatevideo-video-history">
                                <?php echo $this->partial('_video-manage-item.tpl', array('video' => $item, 'manageType' => 'playlist')); ?>
                            </li>
                        <?php elseif ($item->getType() == 'ynultimatevideo_playlist'): ?>
                            <li class="ynultimatevideo_list_most_item ynultimatevideo-playlist-history">
                ...
                <?php if ($this->paginator->getCurrentItemCount() < $totalItem) : ?>
	...
	<div>
		<?php $url = $this->url(array('action' => 'removeall', 'type' => 'ynultimatevideo_video'), 'ynultimatevideo_history', true);?>
		<a href="<?php echo $url?>" class="smoothbox button"><?php echo $this->translate("Clear History"); ?></a>
	</div>
<?php else: ?>
	<div class="tip">
		<span>
			<?php echo $this->translate('You do not have any items in your history.'); ?>
		</span>
	</div>
<?php endif; ?>
