  ...
  getTray : function() {
    if( !$type(this.elements.tray) ) {
      ...
      if( !$type(this.elements.tray) ) {
        ...
        }).inject($('ynfeed_addwithfriends'), 'after');
      }
    }
  ...
  attach : function() {
    ...
    if( this.options.overText ) {
      new Composer.OverText(this.elements.body, $merge({
        textOverride : this._lang("What's on your mind?"),
        poll : true,
        positionOptions: {
...
Composer.Plugin.Interface = new Class({
  ...
  reset : function() {
  	if ($('fly_ynfeed_composer')){
  		$('fly_ynfeed_composer').show();
  	}
  ...
  makeActivator : function() {
    if( !this.elements.activator )
    {
      ...
      }).inject(this.getComposer().getMenu(), 'top');
    }
  },
      ...
      this.elements.menuClose = new Element('a', {
        ...
        'events' : {
          'click' : function(e) {
            ...
            $('compose-menu').setStyle('overflow','visible');
          }.bind(this)
        }
