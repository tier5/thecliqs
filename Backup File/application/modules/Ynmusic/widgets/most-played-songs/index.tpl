...
<div id='ynmusic_listing_album_<?php echo $this->identity;?>'>
  <?php if( $this->paginator->getTotalItemCount() > 0 ):?>
	  <ul class="song-items music-items clearfix">
	  	<?php $page = $this->paginator->getCurrentPageNumber();?>
	  	<?php $itemPerPage = $this->paginator->getItemCountPerPage();?>
	  	<?php $count = 1 + (($page - 1)*$itemPerPage);?>
		<?php foreach ($this->paginator as $item) :?>
		<li id="<?php echo $item->getGuid()?>" class="song-item music-item <?php if ($count <= 3) echo "range"?>">
			<div class="song-count"><?php echo $count?></div>
			<?php echo $this->partial('_song_view.tpl', 'ynmusic', array('item' => $item));?>
		</li>
		<?php $count++;?>
		<?php endforeach;?>
	  </ul>
  ...
  <?php else: ?>
	  <div class="tip">
	    <span>
	      <?php echo $this->translate('No songs found.');?>
	    </span>
	  </div>
  <?php endif; ?>
</div>
