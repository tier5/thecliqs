...
<?php echo $this->htmlLink(
    array('route' => 'admin_default', 'module' => 'ynmusic', 'controller' => 'migration'),
    ...
    <table class='admin_table'>
        ...
        <?php foreach ($this->paginator as $item): ?>
            <tr id="import-item_<?php echo $item->import_id?>">
                ...
                <td><?php echo $this->translate(substr($item->item_type, 8))?></td>
				...
				<?php $canUpdate = Engine_Api::_()->ynmusic()->canUpdateImport($newItem, $oldItem);?>
				<?php if ($canUpdate) :?>
				<?php echo $this->htmlLink(
				    array('route' => 'admin_default', 'module' => 'ynmusic', 'controller' => 'migration', 'action' => 'update-songs', 'id' => $item->import_id),
				    $this->translate('update songs'),
               	...
               	<?php if (!in_array($item->status, array('processing','updating'))) :?>
               		<?php echo $this->htmlLink(
					    array('route' => 'admin_default', 'module' => 'ynmusic', 'controller' => 'migration', 'action' => 'remove', 'id' => $item->import_id),
...
function removeSelected(){
    ...
    $('multiselect').action = '<?php echo $this->url(array('module'=>'ynmusic','controller'=>'migration','action'=>'multiremove'), 'admin_default', true) ?>';
