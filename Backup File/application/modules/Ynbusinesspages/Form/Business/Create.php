<?php
class Ynbusinesspages_Form_Business_Create extends Engine_Form
{
	protected $_formArgs;
  ...
  public function init()
  {
	...
	$isMobile = Engine_Api::_() -> ynbusinesspages() -> isMobile();
	...
	if( !$this->_item ) {
      $defaultCustomFields = new Ynbusinesspages_Form_Business_FieldsParent($defaultFormArgs);
	}
	...
	//Custom field
    if( !$this->_item ) {
      $customFields = new Ynbusinesspages_Form_Business_Fields($this -> _formArgs);
	}
    if( get_class($this) == 'Ynbusinesspages_Form_Business_Create' ) {
	...
	if($this -> _package -> allow_owner_add_customfield)
	{
	...
	if(!$isMobile)
	{
	    $this->addDisplayGroup(array('preview', 'submit_button', 'save_draft', 'cancel'), 'buttons', array(
	      'decorators' => array(
	        'FormElements',
	        'DivDivDivWrapper',
	      ),
