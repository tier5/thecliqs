...
class Timeline_Widget_PageFeedController extends Engine_Content_Widget_Abstract
{
    ...
    public function indexAction()
    {
        ...
        if (Engine_Api::_()->core()->hasSubject()) {
            ...
            if (!$subject->authorization()->isAllowed($viewer, 'view') || !($subject instanceof Page_Model_Page)) //!in_array($subject->getType(), Engine_Api::_()->timeline()->getSupportedItems()))
            {
        ...
        $this->view->length = $length = $request->getParam('limit', 10/*Engine_Api::_()->getApi('settings', 'core')->getSetting('activity.length', 15)*/);
        ...
        $this->view->privacy_type = $privacy_type = ($subject) ? $subject->getType() : 'page';
        $privacy = array();
